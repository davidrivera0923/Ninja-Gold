...
app = Flask(__name__)
app.secret_key = "Welcome to Ninja_Gold"
...
def process_gold():
    ...
    if gold_this_turn >= 0:
        new_message =f"<p class='text-success'>Yay you won{gold_this_turn} from {location} {current_time}</p>"
        session['activities'] += new_message
    elif gold_this_turn < 0:
        new_message =f"<p class='text-danger'>Oh no you lost{gold_this_turn} from {location} {current_time}<p>"
        session['activities'] += new_message
